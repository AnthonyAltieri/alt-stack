...
pub fn openapi_to_rust_code(openapi: &Value, options: &GenerationOptions) -> String {
    ...
    for schema_name in topological_sort_schemas(&schemas) {
        if let Some(schema) = schemas.get(&schema_name) {
            let _ = state.ensure_named_schema(&schema_name, schema);
        }
    }
    ...
    if options.include_routes {
        ...
        if !routes.is_empty() {
            for route in &routes {
                ...
                request_modules.push(render_request_module(route));
                response_modules.push(render_response_module(route));
            }
        }
    }
...
fn render_request_module(route: &RouteInfo) -> Vec<String> {
    let names = generate_route_schema_names(route);
    let module_name = generate_route_module_name(&route.path);
...
fn render_response_module(route: &RouteInfo) -> Vec<String> {
    let module_name = generate_route_module_name(&route.path);
    let method_name = route.method.to_ascii_lowercase();
...
impl RenderState {
    ...
    fn ensure_named_schema(&mut self, name: &str, schema: &Value) -> String {
        ...
        self.fingerprints.insert(fingerprint, name.to_owned());
...
fn render_union_enum(name: &str, schema: &Value, state: &mut RenderState) -> Vec<String> {
    ...
    for (index, variant_schema) in variants.iter().enumerate() {
        ...
        let variant_type = if variant_schema.get("$ref").is_some() {
            render_non_nullable_type_expression(&variant_name, variant_schema, state)
        } else {
            let ensured = state.ensure_named_schema(&variant_name, variant_schema);
            ensured
        };
