...
export function schemaExportNameToInputAlias(name: string): string {
...
function registerInputSchemaName(
  schemaName: string,
  options?: SchemaToTypeOptions,
): string {
  options?.inputSchemaNames?.add(schemaName);
  return schemaExportNameToInputAlias(schemaName);
}
...
function getRegisteredInputAlias(
  schema: AnySchema,
  options?: SchemaToTypeOptions,
): string | undefined {
  if (!schema || typeof schema !== "object") return undefined;
  ...
  if (schema["type"] === "string" && typeof schema["format"] === "string") {
    ...
    if (customSchemaName) {
      return registerInputSchemaName(customSchemaName, options);
    }
  }
...
/**
 * Converts an OpenAPI schema to a TypeScript type string using input aliases.
 * Used for request schemas (params, query, body, headers) where we want z.input<> types.
 *
 * @example
 * schemaToInputTypeString({ type: 'string' }) // => 'string'
 * schemaToInputTypeString({ type: 'object', properties: { id: { type: 'string' } } }) // => '{ id?: string }'
 */
export function schemaToInputTypeString(
  schema: AnySchema,
  options?: SchemaToTypeOptions,
): string {
  if (!schema || typeof schema !== "object") return "unknown";
  ...
  if ("oneOf" in schema && Array.isArray(schema["oneOf"])) {
    const unionMembers = (schema["oneOf"] as AnySchema[]).map((s) =>
      schemaToInputTypeString(s, options),
    );
    result = unionMembers.length > 1 ? `(${unionMembers.join(" | ")})` : unionMembers[0] ?? "unknown";
  }
  // Handle allOf (intersection)
  ...
  // Handle type-based schemas
    ...
    switch (schema["type"]) {
      case "string": {
        const registeredAlias = getRegisteredInputAlias(schema, options);
        if (registeredAlias) {
          result = registeredAlias;
        } else if (schema["enum"] && Array.isArray(schema["enum"])) {
          // String enum
      ...
      case "boolean":
        result = getRegisteredInputAlias(schema, options) ?? "boolean";
        break;
      case "null":
        result = "null";
        break;
      case "array":
        if (schema["items"]) {
          const itemType = schemaToInputTypeString(schema["items"] as AnySchema, options);
          result = `Array<${itemType}>`;
        } else {
          result = "unknown[]";
        }
        break;
      case "object":
        result = objectSchemaToInputTypeString(schema, options);
        break;
      default:
        // Try to detect object from properties
        if (schema["properties"]) {
...
/**
 * Converts an OpenAPI object schema to a TypeScript object type string using input aliases.
 */
function objectSchemaToInputTypeString(
  schema: AnySchema,
  options?: SchemaToTypeOptions,
): string {
  const properties = schema["properties"] as Record<string, AnySchema> | undefined;
  ...
  if (properties) {
    for (const [propName, propSchema] of Object.entries(properties)) {
      const isRequired = required.has(propName);
      const propType = schemaToInputTypeString(propSchema, options);
      const quotedName = quotePropertyName(propName);
      propertyStrings.push(
        `${quotedName}${isRequired ? "" : "?"}: ${propType}`,
      );
  ...
  if (additionalProperties === true) {
    ...
    const additionalType = schemaToInputTypeString(additionalProperties as AnySchema, options);
    propertyStrings.push(`[key: string]: ${additionalType}`);
  }
