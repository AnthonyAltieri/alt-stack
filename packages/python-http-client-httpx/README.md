...
Generated SDK modules import `alt_stack.http_client` at runtime, so SDK consumers install this package. The generator itself is a development-time dependency.

## Use a generated SDK

```python
from alt_stack.http_client import ApiFailure, ApiSuccess
...
Importing `HttpxApiClient` from the generated module gives one typed method per route. Importing it from `alt_stack.http_client` accepts the same arguments and exposes the untyped `request(method, path, ...)` API with runtime validation.
