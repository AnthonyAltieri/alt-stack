...
import { Router, router, ok, err, initTelemetry, type ExpressBaseContext } from "./index.js";

describe("Express telemetry", () => {
  ...
  describe("ignored routes", () => {
    ...
    it("ignores sub-paths of ignored routes", async () => {
      ...
      await request(app).get("/api/internal/status");
      await request(app).get("/api/internal/status/db");
  ...
  describe("span in handler context", () => {
    it("provides span in ctx when telemetry is enabled", async () => {
      ...
      const app = createServer({ "/api": testRouter }, { telemetry: true });
      const res = await request(app).get("/api/with-span");

      expect(res.status).toBe(200);
      expect(res.body).toEqual({ hasSpan: true });
    ...
    it("span is undefined when telemetry is disabled", async () => {
      ...
      const app = createServer({ "/api": testRouter }, { telemetry: false });
      const res = await request(app).get("/api/without-span");

      expect(res.status).toBe(200);
      expect(res.body).toEqual({ hasSpan: false });
    ...
    it("allows adding custom attributes to span from handler", async () => {
      ...
      const app = createServer({ "/api": testRouter }, { telemetry: true });
      const res = await request(app).get("/api/custom-attrs");
  ...
  describe("custom service name", () => {
    it("uses custom service name from config", async () => {
      ...
      const spans = exporter.getFinishedSpans();
      expect(spans).toHaveLength(1);
      expect(spans[0].instrumentationScope.name).toBe("my-express-api");
    });
  });
});
